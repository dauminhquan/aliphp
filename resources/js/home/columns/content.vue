<template>
    <div id="content-wrapper">
        <data-table title="Danh sách cột chung của các template"
                    :columns="columns"
                    :data="data"
                    :targets="[]"
        ...
        >
                ...
                <div class="modal-dialog modal-full">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal">&times;</button>
                            <h5 class="modal-title">Thông tin chi tiết cột</h5>
                        </div>
                            ...
                            <div class="modal-body">
                                <fieldset class="content-group">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-lg-10">
                                            <textarea class="form-control" rows="5" required v-model="info.name"></textarea>
                                        </div>
                                    </div>
                                    <div class="form-group">
            ...
            <div id="create-column" class="modal fade">
                <div class="modal-dialog modal-full">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal">&times;</button>
                            <h5 class="modal-title">Thêm cột</h5>
                        </div>
                            ...
                            <div class="modal-body">
                                <fieldset class="content-group">
                                    ...
                                    <div class="form-group">
                                        <label class="control-label col-lg-2 text-bold" >Tên cột</label>
                                        <div class="col-lg-10">
                                            <textarea class="form-control" v-model="create.name" rows="5" required></textarea>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label class="control-label col-lg-2 text-bold" >Tên cột trong bảng sản phẩm</label>
            ...
            <div id="create-many-column" class="modal fade">
                <div class="modal-dialog modal-full">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal">&times;</button>
    ...
    export default {
        ...
        data(){
            return {
                ...
                buttonConfig: [
                    ...
                    {
                        text: 'Thêm nhiều cột 1 lúc',
                        className: 'btn bg-danger-400',
                        action: function(e, dt, node, config) {

                            $('#create-many-column').modal('show')
                        }
                    }
                ],
                deleting: false,
        ...
        methods: {
            ...
            getData(perPage=500,page=1){
                var vm = this
                axios.get('/api/columns'+'?size='+perPage+'&page='+page).then(data => {
            ...
            updateItem(){
                let vm = this
                vm.waiting =true
                axios.put(`/api/columns/${vm.info.id}`,vm.info).then(data=>{
                    vm.getData()
                    alert('Thành công')
                    vm.waiting =false
            ...
            createItem(){
                let vm = this
                vm.waiting =true
                axios.post(`/api/columns`,vm.create).then(data=>{
                    vm.getData()
                    vm.waiting =false
                    alert('Thành công')
            ...
            createListItem(){
                let vm = this
                vm.waiting =true
                let list = vm.list_columns.split(';')
                let list_create = []
                list.forEach(item => {
                    let t = item.split(':')
                    if(t.length > 0)
                    {
                        ...
                        create.name = t[0]
                        create.product_column = t[1]
                        list_create.push(create)
                ...
                axios.post(`/api/create-many-columns`,{
                    columns: list_create
                }).then(data=>{
                    vm.getData()
                    vm.waiting =false
                    ...
                    if(data.data.list_err.length > 0)
                    {
                        ...
                        data.data.list_err.forEach(i => {
                            keyError.push(i.name)
                        })
                        alert('Thành công! Danh sách column đã tồn tại rồi: '+keyError.toString())
                    }
                    else {
                        alert('Thành công')
                    }
            ...
            deleteItem(){
                ...
                {
                    let indexOf = -1
                    axios.delete('/api/columns'+'/'+vm.primaryKeyDelete).then(data => {
            ...
            deleteListItem() {
                let vm = this
                vm.deleting = true
                axios.delete('/api/delete-many-columns',{
                    params:{
            ...
            getColumnInfo(id){
                let vm = this
                axios.get(`/api/columns/${id}`).then(data => {
